                  ...
                  GestureDetector(
                    onTap: () {
                      if (EditUserInfoType.self == type) {
                        ...
                        ProfileSvc.updateUserInfo(u).then((res) {
                          if (res.statusCode == 200) {
                            if (res.data['code'] == 0) {
                              toast('保存成功！');
                              Provider.of<AuthProvider>(context, listen: false).setUserInfo(u);
                              Navigator.pop(context);
                            } else {
                              toast(res.data['msg']);
                            }
                      ...
                      if (EditUserInfoType.create == type || EditUserInfoType.edit == type) {
                        ...
                        if(EditUserInfoType.create == type) {
                          ProfileSvc.createRelation(r).then((res) {
                            if (res.statusCode == 200) {
                              if (res.data['code'] == 0) {
                                toast('创建成功！');
                          ...
                          r.id = userInfo.id;
                          ProfileSvc.updateRelation(r).then((res) {
                            if (res.statusCode == 200) {
                              if (res.data['code'] == 0) {
                                toast('保存成功！');
                                Navigator.pop(context, { 'refresh': true });
