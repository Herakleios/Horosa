  ...
  void initState() {
    ...
    fluwx.addSubscriber((response) async {
      if (response is WeChatAuthResponse) {
        WechatSvc.getAccessToken(response.code!).then((res) {
          if (res.statusCode == 200) {
            if (res.data['code'] == 0) {
              String token = res.data['data']['token'];
              context.read<AuthProvider>().setToken(token);
              localStorage.write(AppKeys.accessToken, token).then((_) {
                ProfileSvc.getUserInfo().then((res) {
                  context.read<AuthProvider>().setUserInfo(UserInfo.fromJson(
                  ...
                  child: GestureDetector(
                    ...
                    child: Column(
                      children: [
                        ...
                        Padding(
                          padding:
                              EdgeInsets.only(top: 24.w, left: 16.w, right: 16.w),
                          child: Row(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () {
                              ...
                              if(!RegExp(r'^[a-zA-Z0-9]{6,20}$').hasMatch(_accountController.text)){
                                toast(context: context, '账号格式为字母 + 数字，长度6 ~ 20');
                                return;
                              }
                              ...
                              if(!RegExp(r'^[a-zA-Z0-9]{8,}$').hasMatch(_passwordController.text)){
                              ...
                              ProfileSvc.register(_accountController.text, _passwordController.text, _passwordConfirmController.text).then((res) {
                                if(res.statusCode == 200) {
                                  Log.d(res.data);
                                  if(res.data['code'] == 0) {
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () {
                              ...
                              ProfileSvc.login(_accountController.text, _passwordController.text).then((res) {
                                if(res.statusCode == 200) {
                                  Log.d(res.data);
                                  if(res.data['code'] == 0) {
                                    String token = res.data['data']['token'];
